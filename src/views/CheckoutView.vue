...
const submitOrder = async () => {
  ...
  try {
    // MAGIA: Unimos el país a la dirección para el backend
    ...
    // 2. Lógica de Contacto (WhatsApp vs Email)
  ...
  <div class="bg-brand-black min-h-screen pt-20 pb-20">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
      ...
      <div class="flex flex-col lg:flex-row gap-12">
        <div class="flex-1">
          <h2 class="text-2xl font-serif-elegant text-brand-white mb-8 tracking-wider uppercase">Detalles de Envío</h2>
          ...
          <form id="checkout-form" @submit.prevent="submitOrder" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <input v-model="form.customerName" type="text" placeholder="Nombre Completo" required class="w-full bg-transparent border-b border-brand-white/20 px-0 py-3 text-brand-white text-sm font-sans-luxury focus:outline-none focus:border-brand-gold transition-colors placeholder:text-brand-white/30 uppercase tracking-wider">
              <input v-model="form.phoneNumber" type="tel" placeholder="Teléfono / WhatsApp" required class="w-full bg-transparent border-b border-brand-white/20 px-0 py-3 text-brand-white text-sm font-sans-luxury focus:outline-none focus:border-brand-gold transition-colors placeholder:text-brand-white/30 uppercase tracking-wider">
            </div>
            ...
            <div class="border-b border-brand-white/20 py-2">
              <label class="text-[10px] text-brand-white/50 uppercase tracking-[0.2em] block mb-1">Destino del envío</label>
              <select v-model="form.country" class="w-full bg-transparent text-brand-white text-sm font-sans-luxury focus:outline-none focus:text-brand-gold transition-colors cursor-pointer uppercase tracking-wider">
            ...
            <textarea v-model="form.notes" rows="3" placeholder="Notas adicionales (Opcional)" class="w-full bg-transparent border-b border-brand-white/20 px-0 py-3 text-brand-white text-sm font-sans-luxury focus:outline-none focus:border-brand-gold transition-colors placeholder:text-brand-white/30 uppercase tracking-wider resize-none"></textarea>
          </form>
        </div>

        <div class="w-full lg:w-1/3">
          <div class="border border-brand-white/10 bg-brand-black/50 p-8 sticky top-24">
            <h3 class="text-xl font-serif-elegant text-brand-gold mb-6 tracking-wider uppercase border-b border-brand-gold/20 pb-4">Resumen</h3>
            <div class="space-y-4 mb-6 max-h-60 overflow-y-auto pr-2">
              <div v-for="item in cartStore.items" :key="item.productId" class="flex justify-between text-sm">
                <span class="text-brand-white/80 font-sans-luxury uppercase tracking-widest flex-1 pr-4">{{ item.quantity }}x {{ item.productName || 'Joya Cushion' }}</span>
                <span class="text-brand-white font-sans-luxury tracking-widest">${{ (item.price * item.quantity).toLocaleString() }}</span>
              </div>
            </div>
            ...
            <div class="border-t border-brand-white/10 pt-4 space-y-3 mb-8">
              ...
              <div class="flex justify-between text-sm">
                <span class="text-brand-white/60 font-sans-luxury uppercase tracking-widest">{{ form.country === 'Colombia' ? 'Envío Nacional' : 'Envío Internacional' }}</span>
            ...
            <button type="submit" form="checkout-form" :disabled="isSubmitting || cartStore.items.length === 0" class="w-full bg-brand-white text-brand-black px-6 py-4 uppercase text-xs font-bold tracking-[0.2em] hover:bg-brand-gold transition-colors duration-300 disabled:opacity-50">
              {{ isSubmitting ? 'Procesando...' : 'Confirmar Pedido' }}
            </button>
            <p v-if="form.country !== 'Colombia'" class="text-center text-brand-white/40 text-[9px] uppercase tracking-widest mt-4">
