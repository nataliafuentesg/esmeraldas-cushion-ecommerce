  ...
  <div class="bg-brand-black min-h-screen pt-20 pb-20">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-5xl">
      ...
      <div class="flex flex-col md:flex-row justify-between items-end border-b border-brand-white/10 pb-6 mb-12">
        <div>
          <h1 class="text-4xl md:text-5xl font-serif-elegant text-brand-white tracking-widest uppercase mb-2">
            Mi Cuenta
          </h1>
          <p class="text-brand-gold font-sans-luxury uppercase text-xs tracking-[0.2em]">
            Bienvenido, {{ authStore.user?.firstName }} {{ authStore.user?.lastName }}
          </p>
        </div>
        <button @click="handleLogout" 
                class="mt-6 md:mt-0 text-brand-white/50 hover:text-brand-white uppercase text-[10px] tracking-[0.2em] underline underline-offset-4 transition-colors">
          Cerrar Sesión
        </button>
      </div>
      ...
      <div>
        ...
        <div v-if="loading" class="text-brand-white/50 font-sans-luxury text-sm tracking-widest uppercase animate-pulse">
          Buscando tesoros...
        </div>

        <div v-else-if="orders.length === 0" class="text-center py-12 border border-brand-white/5 bg-brand-white/5">
          <p class="text-brand-white/60 font-sans-luxury text-sm tracking-widest uppercase mb-6">
            Aún no has realizado ninguna compra.
          </p>
          <RouterLink to="/coleccion/todas" class="bg-brand-white text-brand-black px-8 py-3 uppercase text-[10px] font-bold tracking-[0.2em] hover:bg-brand-gold transition-colors duration-300">
            Explorar Colección
          </RouterLink>
        </div>
        ...
        <div v-else class="space-y-6">
          <div v-for="order in orders" :key="order.id" 
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 pb-4 border-b border-brand-white/10">
              <div>
                <p class="text-brand-gold text-xs font-bold tracking-widest uppercase mb-1">
                  Pedido #{{ order.orderNumber }}
                </p>
                <p class="text-brand-white/50 text-[10px] uppercase tracking-[0.2em]">
                  {{ new Date(order.createdAt).toLocaleDateString() }}
                </p>
              </div>
              <div class="mt-4 md:mt-0 text-right">
                <p class="text-brand-white font-sans-luxury text-lg tracking-widest mb-1">
            ...
            <div class="space-y-3">
              <div v-for="item in order.items" :key="item.id" class="flex justify-between items-center text-sm">
                <p class="text-brand-white/80 font-sans-luxury tracking-widest">
                  {{ item.quantity }}x <span class="text-brand-white">{{ item.product.name }}</span>
                </p>
                <p class="text-brand-white/50 font-sans-luxury tracking-widest">
