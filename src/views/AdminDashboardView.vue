...
<template>
  <div class="bg-brand-black min-h-screen flex flex-col md:flex-row">
    ...
    <div class="md:hidden flex justify-between items-center bg-brand-black border-b border-brand-white/10 p-4 sticky top-0 z-50">
      <h2 class="text-brand-gold font-serif-elegant text-xl tracking-wide">Admin</h2>
    ...
    <aside :class="isMobileMenuOpen ? 'block' : 'hidden'" 
      ...
      <div class="mb-10 hidden md:block">
        <h2 class="text-brand-gold font-serif-elegant text-2xl tracking-wide">Admin</h2>
        <p class="text-[10px] text-brand-white/50 tracking-wide mt-1">Control Central</p>
      </div>
      ...
      <nav class="flex-1 space-y-2">
        ...
        <button @click="selectTab('productos')" :class="activeTab === 'productos' ? 'text-brand-gold bg-brand-white/5 border-l-2 md:border-l-0 md:border-r-2 border-brand-gold' : 'text-brand-white/70 hover:bg-brand-white/5'" class="w-full text-left px-4 py-4 md:py-3 text-xs tracking-wide flex items-center space-x-3 transition-colors">
          <Icon icon="lucide:gem" class="w-5 h-5 md:w-4 md:h-4" /><span>Inventario</span>
        </button>
        <button @click="selectTab('mensajes')" :class="activeTab === 'mensajes' ? 'text-brand-gold bg-brand-white/5 border-l-2 md:border-l-0 md:border-r-2 border-brand-gold' : 'text-brand-white/70 hover:bg-brand-white/5'" class="w-full text-left px-4 py-4 md:py-3 text-xs tracking-wide flex items-center space-x-3 transition-colors">
      ...
      <div class="mt-8 pt-6 border-t border-brand-white/10">
        <button @click="handleLogout" class="w-full text-left px-4 py-3 text-xs tracking-wide text-red-400 hover:text-red-300 hover:bg-red-900/10 flex items-center space-x-3 transition-colors">
