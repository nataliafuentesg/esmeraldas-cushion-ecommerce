...
const fetchProduct = async () => {
  ...
  try {
    ...
    // Mapeo exhaustivo de la data del Backend (DataSeeder)
    product.value = { 
        ...data, 
        gallery: data.images.map((img, i) => ({ id: i, src: img.imageUrl, alt: data.name })),
        mainImage: data.images[0]?.imageUrl 
    };
    
    // Carga de productos para la sección de relacionados
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 xl:gap-24 mb-32">
        ...
        <div class="flex flex-col">
          ...
          <div class="bg-brand-white/[0.03] border border-brand-white/10 p-10 mb-12">
            <h3 class="text-brand-gold text-[10px] uppercase tracking-[0.4em] mb-10 border-b border-brand-gold/20 pb-4">Especificaciones de la Joya</h3>
            ...
            <div class="grid grid-cols-2 gap-y-10 gap-x-8">
              ...
              <div class="flex flex-col">
                ...
                <span class="text-brand-gold text-sm font-bold uppercase tracking-widest">{{ product.stock }} UNIDADES</span>
              </div>
            </div>
          </div>

          <div class="flex flex-col sm:flex-row gap-4">
            <div class="flex items-center border border-brand-white/20 h-16 bg-brand-white/5">
              <button @click="selectedQuantity > 1 ? selectedQuantity-- : null" class="w-14 h-full text-brand-white hover:bg-white/10 transition-colors">-</button>
              <input type="text" v-model="selectedQuantity" readonly class="w-12 text-center bg-transparent text-brand-white font-bold outline-none" />
              <button @click="selectedQuantity < product.stock ? selectedQuantity++ : null" class="w-14 h-full text-brand-white hover:bg-white/10 transition-colors">+</button>
            </div>
            <button @click="addToCart" class="flex-1 bg-brand-primary text-brand-black font-bold uppercase text-[10px] tracking-[0.4em] hover:bg-brand-gold transition-all duration-700 shadow-lg">
        ...
        <div class="max-w-3xl mx-auto bg-brand-white/[0.03] p-12 border border-brand-white/10 relative overflow-hidden">
          ...
          <form @submit.prevent="submitReview" class="space-y-12">
            ...
            <div class="flex flex-col items-center">
              ...
              <div class="flex gap-4">
                <button v-for="i in 5" :key="i" type="button" @click="newReview.rating = i" @mouseenter="hoverRating = i" @mouseleave="hoverRating = 0" class="transform hover:scale-125 transition-transform duration-300">
                  <Icon :icon="i <= (hoverRating || newReview.rating) ? 'material-symbols:star' : 'material-symbols:star-outline'" 
                        class="w-12 h-12 transition-colors" :class="i <= (hoverRating || newReview.rating) ? 'text-brand-gold' : 'text-brand-white/10'" />
                </button>
              </div>
            </div>
            ...
            <button type="submit" :disabled="isSubmitting" 
                    class="w-full h-16 bg-brand-primary text-brand-black font-bold uppercase text-[11px] tracking-[0.5em] hover:bg-brand-gold transition-all flex items-center justify-center">
...
<style scoped>
.hidden-trap { opacity: 0; position: absolute; height: 0; width: 0; z-index: -1; pointer-events: none; }
