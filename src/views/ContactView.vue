  ...
  <div class="bg-brand-black min-h-screen pt-20 pb-20">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-3xl">
      ...
      <div class="text-center mb-16">
        ...
        <p class="text-brand-white/60 font-sans-luxury uppercase text-xs tracking-[0.2em]">
      ...
      <div v-if="isSent" class="border border-brand-gold p-12 text-center bg-brand-gold/5">
        <h2 class="text-2xl font-serif-elegant text-brand-gold mb-4 tracking-wider uppercase">Mensaje Recibido</h2>
        <p class="text-brand-white/80 font-sans-luxury text-sm tracking-widest uppercase">
          Un asesor de Cushion se pondrá en contacto contigo en breve.
        </p>
        <button @click="isSent = false" class="mt-8 text-xs text-brand-white hover:text-brand-gold uppercase tracking-[0.2em] underline underline-offset-4">
          Enviar otro mensaje
        </button>
      </div>
      ...
      <form v-else @submit.prevent="submitContact" class="space-y-8">
        ...
        <div class="text-center pt-4">
          <button type="submit" :disabled="isSubmitting"
                  class="bg-brand-white text-brand-black px-12 py-4 uppercase text-xs font-bold tracking-[0.2em] hover:bg-brand-gold transition-colors duration-300 disabled:opacity-50">
