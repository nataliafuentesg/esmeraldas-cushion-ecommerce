...
<template>
  <div>
    <h3 class="text-2xl font-serif-elegant text-brand-white tracking-wide mb-8 border-b border-brand-white/10 pb-4">Bandeja de Entrada</h3>
    
    <div v-if="loading" class="text-brand-gold tracking-wide text-xs animate-pulse">Cargando mensajes...</div>
    
    <div v-else-if="messages.length === 0" class="text-brand-white/50 text-sm font-sans-luxury tracking-wide">
    ...
    <div v-else class="space-y-4">
      <div v-for="msg in messages" :key="msg.id" class="border border-brand-white/10 bg-brand-black/50 p-6">
        ...
        <p class="text-[10px] text-brand-white/50 tracking-wide">{{ msg.email }} | Tel: {{ msg.phone }}</p>
